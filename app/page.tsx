      ...
      <div style={{ flex: 1, display: "flex", overflow: "hidden", minHeight: 0 }}>
        <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden", minWidth: 0 }}>
          ...
          <div style={{ display: "flex", flexShrink: 0, height: isMobile ? 42 : 56, background: "#f8fafc", borderBottom: "2px solid #e4e9f2", zIndex: 10 }}>
            ...
            <div style={{ display: "flex", flexShrink: chartVisible ? 1 : 0, zIndex: 11, background: "#f8fafc", flex: chartVisible ? "none" : 1 }}>
              <div className="col-header" style={{ ...(chartVisible ? { width: LIST_W } : { flex: 2, minWidth: LIST_W }), gap: 6 }}>
                <span style={{ opacity: 0.5 }}>⋮⋮</span>{isMobile ? "Task" : "Name / Task"}
                <div className="resize-handle" onMouseDown={e => startResize(e, "list")} />
              </div>
              {!isMobile && <>
                <div className="col-header" style={{ ...(chartVisible ? { width: OWNER_W } : { flex: 1, minWidth: OWNER_W }) }}>
                  Owner
                  <div className="resize-handle" onMouseDown={e => startResize(e, "owner")} />
                </div>
                <div className="col-header" style={{ ...(chartVisible ? { width: STATUS_W } : { flex: 1, minWidth: STATUS_W }) }}>
                  Status
                  <div className="resize-handle" onMouseDown={e => startResize(e, "status")} />
                </div>
                <div className="col-header" style={{ ...(chartVisible ? { width: INFO_W } : { flex: 1, minWidth: INFO_W }) }}>
                  Start
                ...
                <div className="col-header" style={{ ...(chartVisible ? { width: DURATION_W } : { flex: 1, minWidth: DURATION_W }) }}>
                  Days
                </div>
              </>}
            </div>
          ...
          <div style={{ flex: 1, display: "flex", overflow: "hidden", minHeight: 0 }}>
            ...
            >
              {rows.map((row, ri) => {
                ...
                return (
                  <div key={row.id} className={!isPhase ? "rh row-anim" : "row-anim"}
                    ...
                    {/* Name cell */}
                    <div style={{ ...(chartVisible ? { width: LIST_W } : { flex: 2, minWidth: LIST_W, whiteSpace: "normal" }), display: "flex", alignItems: "center", borderRight: "1px solid #edf0f7", padding: `0 8px 0 ${isPhase ? 10 : isEpic ? 20 : 34}px`, gap: 5, overflow: "hidden" }}>
                      {isTask && isDev && (
                        <div draggable
                      ...
                      <span style={{ fontSize: isMobile ? 9 : isPhase ? 11 : isEpic ? 11 : 11, fontWeight: isPhase ? 800 : isEpic ? 700 : 400, color: isPhase ? pm.color : isEpic ? "#1e3a5f" : "#334155", overflow: chartVisible ? "hidden" : "visible", textOverflow: chartVisible ? "ellipsis" : "clip", whiteSpace: chartVisible ? "nowrap" : "normal", flex: 1, letterSpacing: isPhase ? "-0.2px" : "0" }}>
                    ...
                    {/* Owner */}
                    {!isMobile && (
                      <div style={{ ...(chartVisible ? { width: OWNER_W } : { flex: 1, minWidth: OWNER_W }), display: "flex", alignItems: "center", padding: "0 10px", borderRight: "1px solid #edf0f7", fontSize: 10, color: "#64748b", overflow: "hidden", gap: 5, position: "relative" }}>
                        {isTask && <>
                          ...
                          <span style={{ overflow: "hidden", textOverflow: chartVisible ? "ellipsis" : "unset", whiteSpace: chartVisible ? "nowrap" : "normal" }}>{t.owner}</span>
                        </>}
                      </div>
                    )}
                    {/* Status */}
                    {!isMobile && (
                      <div style={{ ...(chartVisible ? { width: STATUS_W } : { flex: 1, minWidth: STATUS_W }), display: "flex", alignItems: "center", padding: "0 10px", borderRight: "1px solid #edf0f7", fontSize: 10, whiteSpace: chartVisible ? "nowrap" : "normal", overflow: chartVisible ? "hidden" : "visible", position: "relative" }}>
                        {isTask && statusStyle && (
                    ...
                    {/* Start */}
                    {!isMobile && (
                      <div style={{ ...(chartVisible ? { width: INFO_W } : { flex: 1, minWidth: INFO_W }), display: "flex", alignItems: "center", padding: "0 10px", borderRight: "1px solid #edf0f7", fontSize: 10, color: "#64748b", whiteSpace: chartVisible ? "nowrap" : "normal", position: "relative", cursor: isDev && isTask ? "pointer" : "default" }}
                        onClick={(e) => {
                          if (isDev && isTask) {
                    ...
                    {/* End */}
                    {!isMobile && (
                      <div style={{ ...(chartVisible ? { width: INFO_W } : { flex: 1, minWidth: INFO_W }), display: "flex", alignItems: "center", padding: "0 10px", borderRight: "1px solid #edf0f7", fontSize: 10, color: "#64748b", whiteSpace: chartVisible ? "nowrap" : "normal", cursor: isDev && isTask ? "pointer" : "default" }}
                        onClick={(e) => {
                          if (isDev && isTask) {
                            const rect = e.currentTarget.getBoundingClientRect();
                    ...
                    {/* Days */}
                    {!isMobile && (
                      <div style={{ ...(chartVisible ? { width: DURATION_W } : { flex: 1, minWidth: DURATION_W }), display: "flex", alignItems: "center", padding: "0 10px", borderRight: "1px solid #e4e9f2", fontSize: 10, color: "#64748b", whiteSpace: chartVisible ? "nowrap" : "normal", fontWeight: 600 }}>
              ...
              {/* Milestone footer */}
              <div style={{ height: isMobile ? 22 : 32, borderTop: "2px solid #e4e9f2", background: "#f8fafc", display: "flex", alignItems: "center", padding: "0 12px", fontSize: isMobile ? 8 : 9, fontWeight: 700, color: "#94a3b8", textTransform: "uppercase", letterSpacing: ".1em", width: "100%", borderRight: chartVisible ? "1px solid #e4e9f2" : "none", flexShrink: 0, gap: 6 }}>
